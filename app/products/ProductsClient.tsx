...
export default function ProductsClient({ products }: { products: any[] }) {
  ...
  async function handleAddToBasket(productId: number) {
    setAdding(productId)
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:'16px'}}>
        {filtered.map((p) => (
          <div key={p.id} style={{background:'white', border:'1px solid #ddd', borderRadius:'6px', padding:'16px', display:'flex', flexDirection:'column', gap:'8px', position:'relative'}}>
            ...
            {p.wasPrice && <div style={{fontSize:'12px', color:'#888', textDecoration:'line-through'}}>£{p.wasPrice.toFixed(2)}</div>}
            ...
            <button
              ...
              style={{background: p.stock === 0 ? '#ddd' : adding === p.id ? '#ffaa00' : '#ffe000', border:'none', padding:'10px', fontWeight:'bold', fontSize:'14px', borderRadius:'4px', cursor: p.stock === 0 ? 'not-allowed' : 'pointer', marginTop:'auto'}}>
              {adding === p.id ? '⏳ Adding...' : p.stock === 0 ? 'Out of Stock' : '🛒 Add to Basket'}
            </button>
          </div>
        ))}
      </div>
